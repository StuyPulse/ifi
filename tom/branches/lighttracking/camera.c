...
void Camera_Handler(void)
{
	...
	unsigned char byte;
	unsigned char i;
	//printf("In camera.c: acks: %d ncks: %d\r\n",camera_acks,camera_ncks);
...
void LED0_Control(int on)
{
  Write_Camera_Serial_Port('L');
  Write_Camera_Serial_Port('0');
  Write_Camera_Serial_Port(1);
  Write_Camera_Serial_Port(on);
}

void Poll_Mode(int state)
{
  Write_Camera_Serial_Port('P');
  Write_Camera_Serial_Port('M');
  Write_Camera_Serial_Port(1);
  Write_Camera_Serial_Port(state);
}
