...
unsigned char Read_Serial_Port_Two(void)
{
	...
	else
	{
		...
		// If the index pointer overflowed, cut-off the
		// high-order bit. Doing this every time is quicker
		// than checking for overflow every time with an if()
		// statement and only then occasionally setting it
		// back to zero. For this to work, the queue size must
		// be a power of 2 (e.g., 16,32,64,128...).
		Rx_2_Queue_Read_Index &= RX_2_QUEUE_INDEX_MASK;

		// since we're about to use the Rx_2_Queue_Write_Index
		// variable, which can also be modified in the
		// interrupt service routine, let's briefly disable
		// the serial port interrupt to make sure that
		// Rx_2_Queue_Write_Index doesn't get altered while
		// we're using it.
		PIE3bits.RC2IE = 0;
