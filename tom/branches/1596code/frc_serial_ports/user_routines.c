...
void Process_Data_From_Master_uP(void)
{
	...
	// This example reads data as it comes in port 1 and
	// then immediatly sends it back out port 1. Using IFI's
	// terminal, type characters into the "To Port ->" box
	// and watch at they reappear in the terminal window when
	// you hit the return key. The characers appear all at once
	// because the terminal stores the characters in a buffer
	// until you hit the return key. If you use a terminal 
	// emulation program, like hyperterm, the characters will
	// appear as you type them because hyperterm sends the
	// character as you type them. If you see double characters
	// in the window it's because hyperterm is echoing the
	// characters for you. You can turn this off by selecting
	// file menu->proberties->settings tab->ASCII Setup button->
	// uncheck the "Echo typed characters locally" box.
	//
	// Also included is an example of how to interpret user
	// entered data from the keyboard to do something useful.
	// For safety reasons the code is commented out.
	...
	if(byte_count > 0)
	{
		...
		// at a time and immediatly send it back out the same
		// serial port 
		for(j = 0; j < byte_count; j++)
		{
			// get the next data byte
			data = Read_Serial_Port_One();
			Write_Serial_Port_One(data);

			// this is how easy it is to interpret keyboard
			// input from the user. For safety reasons this
			// is commented out.
/*
			stdout_serial_port = SERIAL_PORT_ONE;

			// should we go 'f'orward?
			if(data == 'f')
			{
				printf("\r\ngoing forward...\r\n");
				pwm01 = 254;
