...
void Default_Routine(void)
{
  ...
  */
  pwm02 = damp_joy(p1_y,1);
  pwm01 = p2_y = 255 - damp_joy(p1_y,2);   
  /*pwm03 = p3_y;   
    pwm04 = p4_y;   
    pwm05 = p1_x;  
  pwm06 = p2_x;   
...
int damp_joy(int joy_value, int joy_index){
  static int joy_avg[MAX_JOYS][MAX_AVG];
  static int x[MAX_JOYS];
  static int first = 1;
  int i;
  int j;
  int sum;
  ...
  x[joy_index] = x[joy_index]%10;
  for(i=0, i < MAX_AVG, i++){
    sum += joy_avg[joy_index][i];
  }
  joy_avg[joy_index][x[joy_index]] = joy_value;
  x[joy_index]++;
  return (sum/MAX_AVG);
} 
