...
void Process_Data_From_Master_uP(void){
  ...
  if ( j >= 300 )
	...
	Our_User_Code();
      ...
      printf("rpm r: %d\t rpm l: %d\r\n",get_rpm_1k(1),get_rpm_1k(2));
      printf("current r1: %d\t current l1: %d\t current r2: %d\t current l2: %d\r\n",
	     Get_ADC_Result(RF_CURRENT),Get_ADC_Result(LF_CURRENT),
	     Get_ADC_Result(RB_CURRENT),Get_ADC_Result(LB_CURRENT));
...
void Our_User_Code()
{
  ...
  if(user_display_mode)
    {
      User_Mode_byte = Get_ADC_Result(ARM_POT) / 4;
    }
  else
...
void Standard_Drive(robo_state *rs, robo_target *rt, robo_out *ro)
{
  ...
  if(p2_sw_trig) //replace this with a #define when I'm feeling less lazy
    ...
    */
	...
	else if(ARM_DESCORE_SW)
	  {
	    if(!already_pressed)
	      {
		rt->arm_pos = rs->arm_pos + ARM_DESCORE_INC;
		already_pressed = 1;
	      }
	  }
	Set_Arm_Pos(rs,rt,ro);
    ...
    if(!(ARM_SCORE_SW || ARM_DESCORE_SW))
      already_pressed = 0;
}
