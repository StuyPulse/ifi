...
unsigned char EEPROM_Write_Int(unsigned int address, unsigned int data)
{
  unsigned char current;
  int i;
  //      current = (unsigned char) data >> CHARSIZE*(BLOCKSIZE-(i+1));
  current = (unsigned char) data;
  printf("current: %d\n",current);
  data >>= CHARSIZE;
  if(!EEPROM_Write(address+i,current))
    return i;
}
...
unsigned int EEPROM_Read_Int(unsigned int address)
{
  ...
  for(i=BLOCKSIZE-1; i >= 0; i--)
    {
      data <<= CHARSIZE;
      current = EEPROM_Read(address+i);
      data |= (unsigned int)current;
      printf("current: %d, data: %u\n",current,data);
    }
  return data;
}
