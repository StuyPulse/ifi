...
volatile unsigned int left_clock = 0;
volatile unsigned int right_clock = 0;
volatile unsigned int fly_clock = 0;
volatile int left_vel = 0;
volatile int right_vel = 0;
volatile int fly_count = 0;
...
void Initialize_Timer_0(void)  
{
	...
	// assume T0PS3 is the same as PSA!!!
	//T0CONbits.T0PS3 = 1;		// 0: use the prescaler to derive the timer clock
...
void Initialize_Timer_4(void)  
{
	TMR4 = 0x00;			// 8-bit timer 4 register (this is readable and writable)
							//
	PR4	= 249;				// timer 4 period register - timer 4 increments to this 
							// value then resets to zero on the next clock and starts
							// all over again
							//
	T4CONbits.T4OUTPS0 = 1;	// T4OUTPS3 T4OUTPS2 T4OUTPS1 T4OUTPS0
	...
	T4CONbits.T4OUTPS3 = 1;	//    0        0        1        0		1:3 postscaler
							...
							//    1        1        1        1		1:16 postscaler
							//
	T4CONbits.T4CKPS0 = 1;	// T4CKPS1  T4CKPS0
...
{
  /*  if(right_index==SAMPLES_AVERAGED_DRIVE)
    right_index=0;
  else
    right_index++;
  right_count++;
  right_vel[right_index] = right_clock; */
  right_vel = right_clock;
  right_clock=0;
  right_ticks++;
}
...
{
  /*
  if(left_index==SAMPLES_AVERAGED_DRIVE)
    left_index=0;
  else
    left_index++;
  left_count++;
  left_vel[left_index] = left_clock; 
  */
  left_vel = left_clock;
  left_clock=0;
  left_ticks++;
