...
#define NEAR_DISTANCE (params.velocity * .5)
      init_controller(&ps,1000,NEAR_DISTANCE,0,1,0,1,1000,0);
  ...
  int run(robo_state *rs, robo_target *rt)
  {
    ...
    float current_distance = params.distance - (rs->pos_L - initial.pos_L + rs->pos_R - initial.pos_R) / 2.0;
    float vel_change;
	...
	if(params.heading == 0)
	  {
	    vel_change = HEAD_P_GAIN(rs->pos_L - rs->pos_R);
	      printf("rs->vel_R: %d, rs->vel_L: %d\r",(int)rs->pos_R,(int)rs->pos_L,(int)rs->vel_R,(int)rs->vel_L);

  printf("rt->vel_L: %d\t rt->vel_R: %d\r", (int)rt->vel_L, (int)rt->vel_R) ;

	    printf("vel change: %d\r",(int)vel_change);
	    rt->vel_R += vel_change;
	    rt->vel_L -= vel_change;
