...
void Servo_Track(void)
{
  ...
  static short every_other = FALSE;
  int temp_pan_servo;
  int temp_tilt_servo;
  int servo_step;
  int pan_error;
      ...
      else
	{
	  ...
	  if(loop_count > SEARCH_DELAY_DEFAULT)
	      ...
	      else
		{
		  ...
		  if((!every_other && temp_pan_servo >= PAN_MAX_PWM_DEFAULT) ||
		     (every_other && temp_pan_servo <= PAN_MIN_PWM_DEFAULT))
		    {
		      //temp_pan_servo = PAN_MIN_PWM_DEFAULT;
...
int Target_Distance()
{
  return (TARGET_HEIGHT - CAMERA_HEIGHT)/tan(TILT_ANGLE_RADS);
}
