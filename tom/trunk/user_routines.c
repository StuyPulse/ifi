...
int mckee_auton(robo_state *rs, robo_target *rt)
{
  ...
  switch(step)
	...
	step++;
      break;
    case 1:
      if(rs->pos_L + rs->pos_R - pos < 360)
	  rt->vel_L = rt->vel_R = 127;
      else
	step++;
      break;
    case 2:
  ...
  if(step != laststep)
    {
      printf("Next step: %d\r",step);
      local_time = 0;
      pos = (rs->pos_L + rs->pos_R);
    }

  printf("Current step: %d\r",step);
  local_time += 26;
  global_time += 26;
  step = laststep;
  return 0;
