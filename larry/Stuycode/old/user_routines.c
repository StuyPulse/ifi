...
{
  static int grabber = CLOSED;
  static int pump_timer = 0;
  RIGHT_DRIVE = RIGHT_IN;
  LEFT_DRIVE = LEFT_IN;
  UPPER_ARM = UPPER_ARM_IN;

  if(RIGHT_FLIP)
    REVERSE_PWM(RIGHT_DRIVE);
  if(LEFT_FLIP)
    REVERSE_PWM(LEFT_DRIVE);
  if(UPPER_ARM_FLIP)
    REVERSE_PWM(UPPER_ARM);

  if(BALL_GRABBER_OPEN)
    grabber = OPEN;
  if(BALL_GRABBER_CLOSE)
    grabber = CLOSED;

  BALL_GRABBER = grabber;

  LEFT_WING = LEFT_WING_IN;
  RIGHT_WING = RIGHT_WING_IN;
  LOWER_ARM_FWD = LOWER_ARM_IN;
  LOWER_ARM_REV = !LOWER_ARM_IN;
  GOAL_GRABBER = GOAL_GRABBER_IN;

  //PUMP = PUMP_IN;
  if(pump_timer % 1000 < 250 || p1_sw_trig)
    {
      PUMP = 1;
    }
  else
    PUMP=0;
  pump_timer++;
  //printf("Pump: %d\r",Get_Analog_Value(PUMP_IN));
  ...
  */
  ...
  //Relay 4 fwd = UNKNOWN
  //Relay 4 rev = Lance
  relay1_fwd = !rc_dig_in15;
  relay2_fwd = p1_sw_trig;
  relay1_rev = p2_sw_trig;
 /*---------- 1 Joystick Drive ----------------------------------------------
  *--------------------------------------------------------------------------
